...
function MeetingForm({setFormValue}) {
    ...
    const onCreateClick=async()=>{
        ...
        await setDoc(doc(db,'MeetingEvent',id),{
            ...
            toast('New Meeting Event Scheduled');
  ...
  return (
    <div className="p-8">
      <Link href={'/dashboard'}><h2 className="flex gap-4">
        <ChevronLeft /> Cancel
      </h2></Link> 
      <div className="mt-4">
      ...
      <div className="flex flex-col gap-3 my-4">
        ...
        <div className="grid grid-cols-4 gap-3">
            {LocationOption.map((option,index)=>(
                ...
                className={`border cursor-pointer flex flex-col
                 justify-center items-center rounded-lg p-3 hover:bg-blue-100
                  hover:border-primary ${location==option.name && 'bg-blue-100 border-primary'}`} onClick={()=>setLocationType(option.name)}>
        ...
        <div className="flex justify-evenly">
            {ThemeOptions.map((color,index)=>(
                <div key={index} className={`h-5 w-5 ${themeColor==color && 'border-4 border-black'}`} style={{backgroundColor:color}} onClick={()=>setThemeColor(color)}>

                </div>
            ))}
        </div>
      </div>
      <Button className='w-full mt-9' onClick={()=>onCreateClick()} disabled={(!eventName||!duration||!locationType||!locationUrl)}>Create</Button>
    </div>
  );
}
